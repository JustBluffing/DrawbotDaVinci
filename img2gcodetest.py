...
def write_paths_to_gcode(fname, paths):
    # Lift time in seconds. Alternatively control with Z axis movement and accel time in UGS.
    servo_wait_time = 0.8
    pen_up = True
    gcodestring = "M64 P0\nM64 P1\nM64 P2\nM64 P3\n\nG21\nF1000\n"
    for i, path in enumerate(paths):
        ...
        for j, ele in enumerate(path):
            ...
            y2 = np.imag(ele.end)
            if j == 0:
                gcodestring += f"\nM280 P0 S60 G00 Z100"
                gcodestring += f"\nG04 P{servo_wait_time}"
                gcodestring += f"\nG01 X{int(x1)} Y{int(y1)}"
                gcodestring += f"\nM280 P0 S35 G00 Z0"
                gcodestring += f"\nG04 P{servo_wait_time}"
                pen_up = False
            else:
...
@click.option("--minx", default=0, help="minimum x")
@click.option("--maxx", default=220, help="maximum x")
@click.option("--miny", default=0, help="minimum y")
@click.option("--maxy", default=140, help="maximum y")
...
def run(
    ...
    if autotrace:
        ...
        if skeleton:
            ...
            cmd = f"potrace -b svg -o potrace.svg skeleton_negate.png"
            log.debug(cmd)
            subprocess.run(cmd.split())
        else:
            cmd = f"{imconvert} {file} -resize {width}x{height} -background White -gravity center -extent {width}x{height} -threshold {threshold}%% thresholded.png"
    ...
    cmd = f"{imconvert} final.svg -flip final.png"
    log.debug(cmd)
    subprocess.run(cmd.split())
